...
--------------
A simplified GUI application for batch converting MP4 files to optimized GIFs.

Features:
- Select single or multiple MP4 files for conversion
- Convert videos to optimized GIFs (under 5MB)
- Convert videos to high-fidelity GIFs (under 25MB)
- Progress tracking
...
try:
    from src.VI_utils.video_utils import video_to_gif, get_video_info
...
class VideoMP4ToGifApp(ctk.CTk):
    def __init__(self):
        super().__init__()
    ...
    def create_ui(self):
        ...
        ctk.CTkLabel(select_frame, text="MP4 Files Selection", font=("Helvetica", 16, "bold")).grid(row=0, column=0, columnspan=2, pady=(5, 10), sticky="w")
        
        info_text = "Converting MP4 files to two types of GIFs:"
        ...
        ctk.CTkButton(
            ...
            height=40,
            font=("Helvetica", 14)
        ).grid(row=3, column=0, padx=(10, 5), pady=5, sticky="w")
        ...
        ctk.CTkButton(
            ...
            hover_color="#C84744",
            font=("Helvetica", 14)
        ).grid(row=3, column=1, padx=(5, 10), pady=5, sticky="w")
        ...
        self.status_message = ctk.CTkLabel(
            self.status_frame, 
            text="Select MP4 files to convert to GIFs", 
            font=("Helvetica", 14),
            height=30
        )
    ...
    def update_file_list(self):
        ...
        ctk.CTkLabel(header_frame, text="Optimized GIF", font=("Helvetica", 12, "bold")).grid(row=0, column=1, padx=5, pady=5)
        ctk.CTkLabel(header_frame, text="Hi-Fi GIF", font=("Helvetica", 12, "bold")).grid(row=0, column=2, padx=5, pady=5)
        ...
        for i, job in enumerate(self.jobs):
            ...
            gif_label.grid(row=0, column=1, padx=5, pady=5)
            ...
            hifi_label.grid(row=0, column=2, padx=5, pady=5)
    ...
    def start_conversion(self):
        ...
        if not self.jobs:
            messagebox.showinfo("No Files", "Please select at least one MP4 file to convert.")
    ...
    def conversion_thread(self):
        ...
        try:
            ...
            # All jobs completed
            if all(job.gif_success and job.hifi_gif_success for job in self.jobs):
                self.update_status("All conversions completed successfully!", True)
            elif any(job.gif_success or job.hifi_gif_success for job in self.jobs):
    ...
    def create_hifi_gif(self, job: ConversionJob) -> bool:
        ...
        try:
            ...
            if not os.path.exists(job.input_path):
                print(f"ERROR: MP4 file does not exist: {job.input_path}")
            ...
            while current_attempt < max_attempts and not success:
                ...
                video_info = get_video_info(job.input_path)
...
def main():
    print("Starting MP4 to GIF Converter App")
    app = VideoMP4ToGifApp()
    app.mainloop()
